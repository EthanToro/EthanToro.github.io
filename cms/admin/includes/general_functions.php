...
function CheckLogin($invert=false){
	if($invert){
		if(isset($_SESSION['admin_user_obj'])) {
...
function tabDisabled($page){
	global $user,$cms;
	if($user->getPermissions()->getAccessLevel()!=2){
		return !$cms->getSetting($page.'Tab');
	}else return false;
}


function niceditUploadUri(){
return "assets/nicEdit/nicUpload.php";
}

function getUser(){
	CheckLogin();
	if(isset($_SESSION['admin_user_obj'])) {
		return $_SESSION['admin_user_obj'];
	}else{
		kickToCurb();
	}
}
