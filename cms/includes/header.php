...
<div id="wrapper">
    ...
    <nav>
        ...
        if ($cms->getSetting('commentsTab') || ($user->getPermissions()->getAccessLevel() == 2)) {
            echo '<a href="comments.php" class="' . ((!$cms->getSetting('commentsTab')) ? " evil " : "") . 'tab comment' . ((strpos($uri, 'comments')) ? ' selected' : '') . '">Comments</a>';
        }
        if ($cms->getSetting('usersTab') || ($user->getPermissions()->getAccessLevel() == 2)) {
            echo '<a href="user.php" class="' . ((!$cms->getSetting('usersTab')) ? " evil " : "") . 'tab user' . ((strpos($uri, 'user.php')) ? ' selected' : '') . '">Users</a>';
        }
        ...
        <?php }*/ ?>
        <a href="index.php"
           class="tab home<?php echo((strpos($uri, 'index.php') || preg_match('/\/$/', $uri)) ? ' selected' : ''); ?>">Home</a>
    </nav>
